...
export function s32(data: string): string {
    ...
    const sliced = data.toString().slice(2);
...
export async function loadContract(name: string, address: string, provider: any, abi: any = null, sender: ethers.Signer | null = null): Promise<ethers.Contract> {
    if (abi === null) {
        abi = await loadAbi(name); 
    }
